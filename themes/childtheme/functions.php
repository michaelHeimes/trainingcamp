...
foreach($json_array as $item){
    $dataObject = $item; 
    $endDate = new DateTime($dataObject[$endDateKey]);
    $startDate = new DateTime($dataObject[$startDateKey]);
    $endDate->setTimezone( $UTC );
    $startDate->setTimezone( $UTC );
    if($endDate < $now || $startDate <= $now || !sizeof($dataObject['attributes']) || !$dataObject['start_date'] || !$dataObject['end_date']) {
...
add_action( 'wpcf7_before_send_mail', 'limit_subject_contactform' );

function limit_subject_contactform( $WPCF7_ContactForm ) {
        ...
        $subject = $posted_data['message'];           
        $mail= $WPCF7_ContactForm->prop('mail');
        $trim = substr($subject, 0, 200);
        $body = $mail['body'];
        
        $mail['body'] = str_replace("[message]",$trim,$body);                
        //debug_to_console($mail);die;
        // Save the email body
...
function custom_email_subject( $formated_subject, $order ){
    ...
    foreach ( $order->get_items() as $item_id => $item ) {
        if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) { 
            ...
            $item_sku = $item_sku.' '.$product->get_sku();         
        }
    }    

    return sprintf( __('!Registration %s %s', 'woocommerce'), 
        $order->get_formatted_billing_full_name(),$item_sku
    );
}
