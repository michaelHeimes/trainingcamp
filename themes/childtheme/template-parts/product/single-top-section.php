...
$background_description=get_field('product_top_description_background');
$product_description_thumb_url=get_the_post_thumbnail_url(get_the_ID(),'prod_single_img');
if (isset($background_description['sizes']['prod_single_img'])){
    $description_background_url=$background_description['sizes']['prod_single_img'];
}elseif($product_thumb_url){
    $description_background_url=$product_description_thumb_url;
}else{
    $description_background_url=get_template_directory_uri().'/slice/dist/images/course-details-bg.jpg';
}
...
<div>
<div class="top-bg description-product-bg " style="background-image: url('<?= $description_background_url ?>');">
...
<h3 class="word-capitalize training-banner-parent" ><?php echo(get_the_excerpt()); ?></h3>
</div>
<div class="container training-banner">
<?php if (have_rows('product_top_description_group')): ?>
                    <?php while(have_rows('product_top_description_group')): the_row(); ?>
                        <?php
                        $name=get_sub_field('product_top_description_item_name');
                        $value=get_sub_field('product_top_description_item_value');
                        if($name && $value):                
                            ?>              
                               <?php if(strcmp($value,'#pricing-schedules')==0): ?>
                             <?php $value= add_query_arg('product_id',get_the_ID(),get_permalink(get_page_ID_by_page_template('templates/pricing.php'))) ?>
                             <?php endif; ?>                                   
                               <div class="training-banner-item"><a class="product-description-link"href="<?= $value ?>"> <h3><?= $name ?></h3></a></div>
                                <!-- <h5><?= $value ?></h5> -->                        
                            <?php
                        endif; ?>
                    <?php endwhile; ?>
